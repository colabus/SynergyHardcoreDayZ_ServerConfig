class HordeHummerMission extends SurvivorMissions
{
	//Mission related entities
	ItemBase MissionObject;
	EntityAI CrashedHummer;
	...
	void HordeHummerMission()
	{
		ZenModLogger.Log("[STARTED] HordeHummerMission started @ " + m_MissionPosition, "missions");
		...
		//Mission informant
		m_MissionInformant = MissionSettings.BadBoss; // Voice: Onyx

		//Set mission messages
		MissionSettings.StartingMsg = "Can anybody read me? Over?";
		m_MissionMessage1 = "Once again, we have foiled your plans, " + MissionSettings.GoodBoss + ". The military vehicle you sent to scout our location has been destroyed.";
		m_MissionMessage2 = "The driver was alive, but we were able to remedy that situation rather quickly. It seems a single man is no match for a horde of hungry infected.";
		m_MissionMessage3 = "For any remaining survivors out there, feel free to scavange the wreckage for supplies. The crashed hummvee is swarming with infected\n\n** at the " + m_MissionLocation + " military base **\n\nGood luck, if anyone is stupid enough to take up the challenge.";
	...
	void ~HordeHummerMission()
	{
		ZenModLogger.Log("[DESTROY] HordeHummerMission class destroyed.", "missions");
	...
	void SpawnObjects()
	{
		...
		heliPos[0] = heliPos[0] + 7;
		heliPos[2] = heliPos[2] - 7;
		CrashedHummer = EntityAI.Cast(GetGame().CreateObject("ZenMission_Wreck_Hummer", heliPos));
		// StaticObj_Wreck_Uaz_DE

		if (CrashedHummer)
		{
			CrashedHummer.PlaceOnSurface();
			CrashedHummer.SetLifetime(MISSION_TIME);
			
			// Align to surface
			vector transform[4];
			CrashedHummer.GetTransform(transform);
	...
	{
		ZenModLogger.Log("HordeHummer completed.", "missions");
		...
		MissionSettings.FinishingMsg = "Thank you for trying to help rescue our driver, survivor. It's a shame they didn't make it. We've dispatched an airdrop as a reward, it should arrive in a few minutes. If you're still listening to this frequency " + MissionSettings.BadBoss + ", sleep with one eye open. You and your flock of terrorist's days are numbered.";
